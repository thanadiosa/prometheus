...
fetch_lib() {
  ...
  scp -P "$port" -o StrictHostKeyChecking=accept-new -o ConnectTimeout=15 -o BatchMode=yes \
...
while :; do
  ...
  log "could not fetch helper-lib.sh — check access/password; re-enter"
  rm -f "$HELPER_PASS_FILE" ./helper-lib.sh
